...
export default function Register() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(false);
    if (ok) { toast.success("Account created successfully! Welcome to MuseoAI."); navigate("/", { replace: true }); }
    ...
    <div className="min-h-screen bg-[#F8F5F0] flex">
      ...
      <div className="hidden lg:block lg:w-1/2 relative">
        ...
        <div className="absolute inset-0 flex flex-col justify-center px-12">
          ...
          <h2 className="font-display text-4xl text-white font-bold leading-tight mb-4">
            Join a Community<br />
            <span className="text-[#D8B892]">of Explorers.</span>
          </h2>
          <p className="text-[#EDD9BC] leading-relaxed">
            Create guides, save favorites, and chat with AI about every museum in our curated global directory.
          </p>
        </div>
      </div>
      ...
      <div className="w-full lg:w-1/2 flex items-center justify-center px-6 py-12">
        ...
        >
          ...
          <h1 className="font-display text-3xl text-[#4E342E] font-bold mb-1">Create account</h1>
          <p className="text-[#8B857C] text-sm mb-8">Start your museum journey today</p>

          <button
            onClick={() => { loginDemo(); navigate(from, { replace: true }); }}
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div>
              <label className="block text-xs font-semibold text-[#8B857C] uppercase tracking-wide mb-1.5">Full Name</label>
              <input
                value={form.name}
            ...
            <div>
              ...
              <div className="relative">
                ...
                />
                <button type="button" onClick={() => setShowPass(!showPass)} className="absolute right-3 top-1/2 -translate-y-1/2 text-[#8B857C]">
                  {showPass ? <EyeOff className="w-4 h-4" /> : <Eye className="w-4 h-4" />}
                </button>
              </div>
            </div>
            <div>
              <label className="block text-xs font-semibold text-[#8B857C] uppercase tracking-wide mb-1.5">Confirm Password</label>
              <input
                type="password"
                value={form.confirm}
          ...
          <p className="text-center text-[#8B857C] text-sm mt-6">
            ...
            <Link to="/login" className="text-[#A65E2E] font-medium hover:underline">Sign In</Link>
          </p>
        </motion.div>
      </div>
    </div>
